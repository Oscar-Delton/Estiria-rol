...
function renderInicio() {
  const esDev = currentUser && currentUser.rol === 'dev';
...
function renderBanco() {
  const esDev = currentUser && currentUser.rol === 'dev';
  const esAdminBanco = currentUser && (currentUser.rol === 'admin_banco' || esDev);
...
function mostrarTransferencia() {
  ...
  panel.innerHTML = '<h3>💸 Transferir dinero</h3><input type="text" id="transfer-usuario" placeholder="Usuario destino"/><input type="number" id="transfer-monto" placeholder="Monto en £" min="1"/><textarea id="transfer-desc" placeholder="Descripcion obligatoria"></textarea><button class="btn btn-primary btn-full" id="confirmar-transfer">Confirmar transferencia</button><div id="transfer-error" class="hidden" style="color:var(--danger);margin-top:0.5rem"></div>';
...
async function ejecutarTransferencia() {
  ...
  const mostrarErrorTransfer = function(msg) { errorEl.textContent = msg; errorEl.classList.remove('hidden'); };
  if (!usuarioDestino) return mostrarErrorTransfer('Ingresa el usuario destino');
  if (!monto || monto <= 0) return mostrarErrorTransfer('Ingresa un monto valido');
  if (!descripcion) return mostrarErrorTransfer('La descripcion es obligatoria');
  ...
  try {
    const usernameSnap = await getDoc(doc(db, 'usernames', usuarioDestino));
    if (!usernameSnap.exists()) {
    ...
    document.getElementById('banco-panel').innerHTML = '<div style="text-align:center;padding:1rem"><p style="font-size:2rem">✅</p><p>Transferencia exitosa</p><p style="color:var(--text-secondary)">Se enviaron £' + monto.toLocaleString('es-CO') + ' a ' + usuarioDestino + '</p></div>';
  } catch (err) {
...
function mostrarMovimientos() {
  ...
  let movimientos = [];
  const q1 = query(collection(db, 'transacciones'), where('de', '==', currentUser.uid), orderBy('fecha', 'desc'));
  const q2 = query(collection(db, 'transacciones'), where('para', '==', currentUser.uid), orderBy('fecha', 'desc'));
  onSnapshot(q1, function(snap) {
    snap.docs.forEach(function(d) { if (!movimientos.find(function(m) { return m.id === d.id; })) movimientos.push(Object.assign({ id: d.id }, d.data())); });
...
function renderMovimientos(movimientos) {
  ...
  if (!lista) return;
  if (movimientos.length === 0) { lista.innerHTML = '<p style="color:var(--text-secondary)">Sin movimientos aun</p>'; return; }
  movimientos.sort(function(a, b) { return new Date(b.fecha) - new Date(a.fecha); });
  lista.innerHTML = movimientos.map(function(m) {
...
function mostrarImpuestos() {
  ...
  const q = query(collection(db, 'impuestos'), where('uid', '==', currentUser.uid), orderBy('fecha', 'desc'));
...
function mostrarEditarSaldo() {
  const panel = document.getElementById('banco-panel');
  panel.innerHTML = '<h3>✏️ Editar saldo de usuario</h3><input type="text" id="admin-usuario" placeholder="Nombre de usuario"/><input type="number" id="admin-saldo" placeholder="Nuevo saldo en £" min="0"/><button class="btn btn-primary btn-full" id="confirmar-editar-saldo">Aplicar cambio</button><div id="admin-error" class="hidden" style="color:var(--danger);margin-top:0.5rem"></div>';
  document.getElementById('confirmar-editar-saldo').addEventListener('click', async function() {
    ...
    const mostrarErr = function(msg) { errorEl.textContent = msg; errorEl.classList.remove('hidden'); };
    ...
    await addDoc(collection(db, 'transacciones'), { tipo: 'ajuste_admin', de: currentUser.uid, deUsername: currentUser.username, para: uid, paraUsername: username, monto: nuevoSaldo, descripcion: 'Ajuste de saldo por administrador', fecha: new Date().toISOString(), estado: 'completada' });
    document.getElementById('banco-panel').innerHTML = '<div style="text-align:center;padding:1rem"><p style="font-size:2rem">✅</p><p>Saldo actualizado</p><p style="color:var(--text-secondary)">' + username + ' ahora tiene £' + nuevoSaldo.toLocaleString('es-CO') + '</p></div>';
  });
}
